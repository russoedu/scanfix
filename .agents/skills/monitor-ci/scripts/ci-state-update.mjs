...
function gate () {
  ...
  if (gateType === 'local-fix') {
    const count = Number(getArg('--local-verify-count') || '0')
    const max = Number(getArg('--local-verify-attempts') || '3')
    if (count >= max) {
      return output({
        allowed:          false,
  ...
  if (gateType === 'env-rerun') {
    const count = Number(getArg('--env-rerun-count') || '0')
    if (count >= 2) {
      return output({
        allowed:       false,
        envRerunCount: count,
...
function cycleCheck () {
  ...
  let cycleCount = Number(getArg('--cycle-count') || '0')
  const maxCycles = Number(getArg('--max-cycles') || '10')
  let envRerunCount = Number(getArg('--env-rerun-count') || '0')
